...
Nesta seção são apresentados os resultados da análise dos modelos de regressão logística para os dados da tabela 7.7, que pode ser observada abaixo na Tabela \@ref(tab:tabela)
...
Temos como modelo final $\log(\frac{\theta(x)}{1-\theta(x)})=-1.1747 + 1.2770 * (SexoMasculino) + 1.0545 * (ECG≥ 0,1 ST)$ que foi escolhido pelo resultados 
...
summary(modelo1)
summary(modelo2)
resumo_mod3<-summary(modelo3) # melhor modelo
summary(modelo4)
